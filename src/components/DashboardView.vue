<template>
  ...
  >
    <div class="flex flex-row items-center space-x-4 m-3">
      <div>Input:</div>
      <input class=" rounded text-center bg-lightBlue outline-none " v-model="inputNumber" placeholder="enter here" />
      <select class="flebasis-1/2 rounded shadow-md text-center p-1" :value="selectedInput" @change="changeUnit($event, 'input')">
    ...
    <div class="flex flex-row items-baseline space-x-9 m-3">
      <h1>Output:</h1>
      <h1  class="font-outputDigits text-4xl font-extrabold">{{ calculate }}</h1>
      <select class="flebasis-1/2 rounded shadow-md text-center p-1" :value="selectedOutput" @change="changeUnit($event, 'output')">
        <option v-for="option in availableUnits" :key="option" :value="option">
          {{ option }}
        </option>
      </select>
